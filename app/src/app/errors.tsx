...
function asText(r: ErrRow): string {
  return [
    `[${r.created_at}] ${r.kind} — ${r.user_name ?? r.user_id ?? 'ismeretlen'}`,
    `Üzenet: ${r.message}`,
...
export default function Errors() {
  ...
  const list = needle
    ? rows.filter((r) => `${r.user_name} ${r.kind} ${r.message} ${r.route}`.toLowerCase().includes(needle))
    : rows;
      ...
      <Card>
        ...
        <View style={{ flexDirection: 'row', gap: 8, flexWrap: 'wrap' }}>
          <Btn title={`Összes másolása (${list.length})`} small disabled={list.length === 0}
            onPress={() => void copy(list.map(asText).join('\n\n---\n\n'), `${list.length} bejegyzés a vágólapon.`)} />
        ...
        return (
          <Card key={r.id}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', gap: 8 }}>
              <Body style={{ fontWeight: '700', flex: 1 }}>{r.user_name ?? 'Ismeretlen felhasználó'}</Body>
            ...
            <View style={{ flexDirection: 'row', gap: 8 }}>
              <Btn title="Másolás" small onPress={() => void copy(asText(r), 'A bejegyzés a vágólapon.')} />
