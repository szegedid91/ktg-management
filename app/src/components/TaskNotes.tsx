...
/** canWrite: munkavállalónál akkor igaz, ha a feladat rá van kiosztva — ekkor
 *  ő is írhat (mindig látható) megjegyzést, és a sajátját szerkeszti/törli. */
export function TaskNotes({ taskId, isWorker, canWrite = !isWorker }: { taskId: string; isWorker: boolean; canWrite?: boolean }) {
  ...
  const add = async () => {
    ...
    if (!body) return;
    if (!isWorker && toWorkers && !await confirmDialog(CONFIRM_TITLE, CONFIRM_BODY, 'Igen, rögzítem')) return;
    insertRow('task_notes', { task_id: taskId, body, visible_to_workers: isWorker ? true : toWorkers });
  ...
  const saveEdit = async (n: TaskNote) => {
    ...
    if (!isWorker && editVis && (!n.visible_to_workers || body !== n.body) && !await confirmDialog(CONFIRM_TITLE, CONFIRM_BODY, 'Igen, mentem')) return;
    updateRow('task_notes', n.id, { body, visible_to_workers: isWorker ? true : editVis });
    setEditId(null);
  };
        ...
        <View key={n.id} style={{ gap: 4, borderLeftWidth: 3, borderLeftColor: n.visible_to_workers ? C.primary : C.border, paddingLeft: S.sm, paddingVertical: 2 }}>
          {editId === n.id ? (
            <View style={{ gap: S.sm }}>
              ...
              {!isWorker ? <Check checked={editVis} onToggle={() => setEditVis(!editVis)} label="A munkavállaló is lássa" /> : null}
