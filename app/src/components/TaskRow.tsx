  ...
  return (
    ...
    >
      ...
      <Text style={{ fontSize: 12, color: C.sub }} numberOfLines={1}>
        {showSite && site ? `📍 ${site.name} · ` : ''}👷 {names.join(', ') || (unassigned ? 'még nincs kiosztva' : '—')} · {hd(task.created_at.slice(0, 10))}{task.done_at ? ` · ${task.status === 'failed' ? '⚠️ lezárva' : '✔ kész'} ${hd(task.done_at.slice(0, 10))}` : ''}
